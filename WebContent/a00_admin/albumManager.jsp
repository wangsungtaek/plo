...
<body>
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div
						...
						<h1 class="h3 mb-3 text-gray-800">앨범 정보</h1>
					</div>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">앨범 상세</h5>
				...
				<div class="modal-body wd-70">
					<table class="table table-bordered mb-0" width="100%" cellspacing="0">
						<tr><th>번호</th><td><input name="alb_no" type="text" readonly="readonly" /></td></tr>
						<tr><th>앨범명</th><td><input name="alb_name" type="text"/></td></tr>
						<tr><th>발매형태</th><td><input name="alb_shape" type="text"/></td></tr>
						<tr><th>발매날짜</th><td><input name="alb_date" type="date"/></td></tr>
						<tr><th>이미지</th><td><input name="alb_img" type="text"/></td></tr>
						<tr><th>소개</th><td><textarea name="alb_intro" rows="" cols="" ></textarea></td></tr>
						<tr><th>아티스트</th><td>
												<select name="art_no">
	...
	<script>
		...
		function detail(no){
			console.log(no);
			xhr.open("get", "${path}/a00_admin/z04_albumDetail.jsp?no="+no, true);
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4 && xhr.status == 200){
					var album = JSON.parse(xhr.responseText);
					$('[name=alb_no]').val(album.alb_no);
					$('[name=alb_name]').val(album.alb_name);
					$('[name=alb_shape]').val(album.alb_shape);
					$('[name=alb_date]').val(album.alb_date);
					$('[name=alb_img]').val(album.alb_img);
					$('[name=alb_intro]').html(album.alb_intro);
					$('[name=art_no]').val(album.art_no);
				}
			};
			xhr.send();
