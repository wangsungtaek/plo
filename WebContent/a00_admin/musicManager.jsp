...
.divScroll { width: 500px; overflow: scroll;}
</style>
</head>
<body>
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div
						...
						<h1 class="h3 mb-3 text-gray-800">음원 정보</h1>
					</div>

					<!-- DataTale -->
					<div class="card shadow mb-4">
						<div class="card-header py-3">
							<h6 class="m-0 font-weight-bold text-primary">음원 조회</h6>
						</div>
						...
						<div class="card-body pb-0">
							<div class="table-responsive">
								...
								<table class="dataTable table table-bordered table-hover" id="dataTable"
									...
									<tbody>
										...
										<tr ondblclick="detail(${music.m_no})">
											<td>${music.m_no}</td>
											<td>${music.m_name}</td>
											<td>${music.m_playnum}</td>
											<td><div style="width: 150px; height: 20px; overflow-x: auto; white-space: nowrap;">
												${music.m_path}</div></td>
											<td><div style="width: 400px; height: 20px; overflow-x: auto; white-space: nowrap;">
												${music.m_lyrics}</div></td>
											<td>${music.alb_name}</td>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">음원 상세</h5>
				...
				<div class="modal-body wd-70">
					<table class="table table-bordered mb-0" width="100%" cellspacing="0">
						<tr><th>번호</th><td><input name="m_no" type="text" readonly="readonly" /></td></tr>
						<tr><th>음원명</th><td><input name="m_name" type="text"/></td></tr>
						<tr><th>재생수</th><td><input name="m_playnum" type="text"/></td></tr>
						<tr><th>파일경로</th><td><input name="m_path" type="text"/></td></tr>
						<tr><th>가사</th><td><textarea name="m_lyrics" rows="" cols="" ></textarea></td></tr>
						<tr><th>앨범명</th><td>
												<select name="m_no">
	...
	<script>
		...
		xhr.open("get", "${path}/a00_admin/z04_albumList.jsp", true);
		xhr.onreadystatechange = function() {
			if(xhr.readyState == 4 && xhr.status == 200){
				var album = JSON.parse(xhr.responseText);
				var show;
				$(album.albumList).each(function(idx, alb){
					show += '<option value="'+alb.alb_no+'">'+alb.alb_name+'</option>';
				});
				$('[name=m_no]').html(show);
			}
		}
		...
		function detail(no){
			console.log(no);
			xhr.open("get", "${path}/a00_admin/z05_musicDetail.jsp?no="+no, true);
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4 && xhr.status == 200){
					var music = JSON.parse(xhr.responseText);
					$('[name=m_no]').val(music.m_no);
					$('[name=m_name]').val(music.m_name);
					$('[name=m_playnum]').val(music.m_playnum);
					$('[name=m_path]').val(music.m_path);
					$('[name=m_lyrics]').html(music.m_lyrics);
					$('[name=m_no]').val(music.m_no);
				}
			};
			xhr.send();
